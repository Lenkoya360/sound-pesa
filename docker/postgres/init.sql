-- PostgreSQL initialization script for Sound Pesa Platform
...
-- Create blockchain status tracking table
CREATE TABLE IF NOT EXISTS blockchain.node_status (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    blockchain VARCHAR(50) NOT NULL,
    is_synced BOOLEAN DEFAULT FALSE,
    current_block BIGINT DEFAULT 0,
    highest_block BIGINT DEFAULT 0,
    peer_count INTEGER DEFAULT 0,
    last_updated TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(blockchain)
);

-- Insert initial blockchain status records
